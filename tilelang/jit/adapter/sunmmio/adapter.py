...
from tilelang.jit.adapter.sunmmio.checks import check_sunmmio_available
from tilelang.jit.adapter.sunmmio.libgen import SunmmioLibraryGenerator
from tilelang.jit.adapter.sunmmio.wrapper import TLSunmmioSourceWrapper
...
class SunmmioKernelAdapter(BaseKernelAdapter):
    ...
    def __init__(
        ...
        # NOTE: need compile mlir to get elf first
        self.lib_generator = SunmmioLibraryGenerator(self.target, self.verbose)
        self.lib_generator.update_lib_code(self.device_kernel_source)
        self.lib_generator.assign_compile_flags(compile_flags)
        self.lib_generator.compile_mlir()

        self.wrapper = TLSunmmioSourceWrapper(self.ir_module, self.device_kernel_source, device_mod, host_mod, pass_configs)
        self.python_wrapper_code = getattr(self.wrapper, "python_wrapper", None)
        self.launcher_cpp_code = getattr(self.wrapper, "launcher_cpp_code", None)
        self.launcher_lib_name = getattr(self.wrapper, "launcher_lib_name", None)
        
        self.lib_generator.update_python_wrapper(self.python_wrapper_code)
        ...
        self.lib_generator.compile_launch_lib()
    ...
    def from_database(
        ...
        adapter.lib_generator = SunmmioLibraryGenerator(adapter.target, adapter.verbose)
    ...
    def _process_dynamic_symbolic(self) -> tuple[dict[tir.Var, tuple[int, int, int]], list[tir.Var]]:
        ...
        def scan_shape(param_indices: list[int]):
        ...
        def scan_stride(param_indices: list[int]):
            for i in param_indices:
                param = params[i]
                if param not in buffer_map:
                    continue
        ...
        input_param_indices = [i for i in range(len(params)) if i not in self.result_idx]
        output_param_indices = [i for i in range(len(params)) if i in self.result_idx]

        # Prefer input tensors as dynamic-symbol sources because result_idx outputs
        # may not exist yet when the wrapper allocates them.
        scan_shape(input_param_indices)
        scan_shape(output_param_indices)
        scan_stride(input_param_indices)
        scan_stride(output_param_indices)

        return dynamic_symbolic_map, dynamic_symbolic_order

    def get_kernel_source(self) -> str | None:
        return self.python_wrapper_code
